...
async function handleStreamingRefresh() {
  ...
  ;(async () => {
    try {
      sendLog({ type: 'info', message: 'Starting referee stats computation from fixture data...' })
      ...
      if (!fixtures || fixtures.length === 0) {
        sendLog({ type: 'info', message: 'No completed fixtures with referee data found' })
        close({ success: true, imported: 0, errors: 0, total: 0, duration: Date.now() - startTime })
        return
      }

      sendLog({ type: 'info', message: `Found ${fixtures.length} completed fixtures with referee data` })
      ...
      const refereeData: Record<string, {
        matches: number; yellowCards: number; redCards: number;
        fouls: number; penalties: number; homeWins: number;
        awayWins: number; draws: number
      }> = {}
      ...
      for (const fixture of fixtures) {
        ...
        if (!refereeData[refName]) {
          refereeData[refName] = { matches: 0, yellowCards: 0, redCards: 0, fouls: 0, penalties: 0, homeWins: 0, awayWins: 0, draws: 0 }
        }
        ...
        if (homeGoals > awayGoals) refereeData[refName].homeWins++
        else if (awayGoals > homeGoals) refereeData[refName].awayWins++
        else refereeData[refName].draws++

        const fixtureEvents = events?.filter(e => e.fixture_id === fixture.id) || []
        for (const event of fixtureEvents) {
          if (event.type === 'Card') {
            if (event.detail === 'Yellow Card') refereeData[refName].yellowCards++
            else if (event.detail === 'Red Card' || event.detail === 'Second Yellow card') refereeData[refName].redCards++
      ...
      sendLog({ type: 'info', message: `Processing ${referees.length} unique referees...` })
      ...
      for (let i = 0; i < referees.length; i++) {
        ...
        sendLog({
          type: 'progress',
          message: `Computing stats for ${refName}...`,
          details: { progress: { current: i + 1, total: referees.length } }
        })
      ...
      sendLog({ type: 'success', message: `Completed: ${imported} referees updated, ${errors} errors (${(duration / 1000).toFixed(1)}s)` })
      close({ success: true, imported, errors, total: referees.length, duration })
    } catch (error) {
      const duration = Date.now() - startTime
