...
async function handleStreamingRefresh() {
  ...
  ;(async () => {
    try {
      sendLog({ type: 'info', message: 'Starting fixture events refresh...' })
      ...
      sendLog({ type: 'info', message: `Found ${fixturesToProcess.length} fixtures needing events (${fixtures.length} total completed)` })
